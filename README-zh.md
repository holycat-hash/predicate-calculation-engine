...
**simulation core 有且只有四层抽象——runtime / entity / calculation / predicate**——任何业务需求必须折叠进这四层，禁止引入第五种概念：没有消息、没有事件总线、没有回调、没有全局函数。core 之上有两类派生物，复用同一四层闭包、不是新概念：**派生消费者 runtime**（render，第二个 runtime，单向消费 sim 写流）与**物化索引 helper**（spatial）。
...
- **D1 单 writer 制** —— 每个 cell 静态归属唯一一个 writer（calculation / runtime 内建 / render extension 三类），注册期检查，冲突即错。
- **D2 写即事件** —— write 一律产生事件，无论值是否变化；「值真的变了」由条件 `changed` 显式表达。
- **D3 batch 不排序** —— batch 交付顺序未定义，消费逻辑必须顺序无关（多重集语义）。
- **D4 副作用封闭** —— calculation 闭包唯一可观测效应经 `ctx`，无 ctx 外 ambient 副作用；与写局部性并列，是执行阶段可自由重排 / 并行的前提。
...
```
src/
  lib.rs             # crate 入口、四层核心 + 派生 runtime / helper 导出、优化档位总览
