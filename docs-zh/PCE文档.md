...
它与**写局部性**是两条正交约束，不可混为一谈：写局部性约束「写到哪」（只能写自己实例的字段），由 `Ctx::write` 不接受实例参数 + D1 静态保证；effect-confinement 约束「ctx 之外还能不能动」（不能）。General calculation 仍是不透明 `Box<dyn Fn>` 闭包，因此 D4 仍是契约；kernel 子集已经降为数据：`KernelIr` 走 SoA `KernelBackend` 派发，`Tier::Kernel` 要求提供 IR 并在注册期校验读写约束，所以 D4 对该子集已是可机检保证。
...
**多次触发（D3 推论一）。** each 交付下，同一 calculation 在一帧内可能运行多次（多条命中）。各次运行基于同一快照；若多次运行写同一字段，折叠顺序未定义，最终值不确定。因此**帧内聚合一律用 batch 或 fold 表达，禁止用 each 做读-改-写累加**。runtime 可在帧末检测「同字段被同一 calculation 的多次运行写入不同值」并告警；告警等级是构建期档位（`Detect`：debug 默认 `Warn`、release 默认 `Silent` 折叠、可选 `Strict` panic），非运行时仲裁。
...
1. **仅 each 交付** —— 合取 scope 当前只支持 `each`；`batch` / `fold` 与合取同用在注册期即报错（同帧门控下「聚什么、聚几次」语义不良定义，刻意拒绝；组合需求经 §6.1 物化中间 entity）。
...
`project` 可投影：`new`、`old`、writer 的实例 id（作为 ref 值交付）、订阅者 own 行字段，以及它们之上与条件侧同一封闭集（new/old/own/常量）的标量四则（如 `new.raw * own.armor`，复用同一份 `Expr` 预编译器）。投影发生在命中之后、O(表达式大小)/交付，不进路由成本预算；不引用其他实例 ⇒ 非 join。交付一律是值快照，不是引用——引用不跨实体，值经由谓词通道流动。
...
- **fold** —— 算子是定型的幺半群集 `{sum, count, min, max}`，刻意不开放自定义幺半群（开放需可逆或重算契约，min/max 是非可逆先例）；`min` / `max` 非可逆，用多重集维护 O(log n)/写；`count` 计的是**不同贡献 cell 的个数**（成员数），`sum` 忽略非数值写入。成员死亡（无写）经 runtime 带外撤销，并在下一帧重投递收缩后的聚合值（§6.3）。同样不能与合取 `&` 同用。
...
定时语义经 `Clock` 的 alarm 机制（内部 timer wheel，O(1) / 帧）：到点 runtime 写 `Clock.alarm = payload`，订阅者 each 触发。接口为 `Runtime::set_alarm(at_frame, payload)`（绝对帧）与 `set_alarm_in(frames, payload)`（相对当前帧）。
...
**开放问题。** 原列五条已收敛：alarm 接口（`set_alarm` / `set_alarm_in`，§6.2）、投影侧四则（已开放，§3.4）、同字段多写检测（构建期 `Detect` 档位，§2）三项已落地；执行层 kernel-IR seam 也已落地：`KernelIr` / `KernelOp`、`Tier::Kernel` 读 / 写校验、SoA `KernelColumn` 批执行，以及可插拔 `KernelBackend` 选择与默认 `ScalarKernelBackend`。`fold` 自定义幺半群与合取 `&` × `batch`/`fold` 两项经评估**刻意保持封闭 / 拒绝**——前者开放需可逆或重算契约（min/max 是非可逆先例），后者同帧门控下聚合语义不良定义，二者的组合需求都经 §6.1 物化中间 entity 表达。剩余前瞻工作是可选硬件 backend：SIMD 代码生成、feature-gated GPU 执行与真实 GPU 驻留策略。
...
- render clock tick → **连续更新**：每 render 帧对每个在场实例运行（相机阻尼、派生 transform、插值），ECS 稠密扫，是 render 的主热路径（插值天生每帧）。连续扫遵循与 sim 同款的**快照读**（本帧连续写帧末统一提交、扫描期互不可见），故跨实例读确定、且 `parallel` feature 下可按实例并行（render 字段单写者，无竞争）。
...
**render-local 临时实体是上条规则刻意留的例外，也正是它说「共享」的原因。** 粒子、飘字、贴花是纯视觉住户，绝不该挤进 sim schema / 写日志 / 谓词。它们是 **render 私有的池化实体，由 render 自管生杀**：独立 id 命名空间（`RenderLocalTypeId` / `RenderLocalId`，绝不冒充 sim `InstanceId`），底座复用与 sim 行同一套类型化 `Column` + `GenSlots` 内核；释放的 id 进池、复用时代际 +1，旧句柄永不误指新住户（与 sim 行同款 ABA 防护）。render 直接持有它们的出生与死亡：`spawn_local` 可来自宿主调用、共享 render reaction、或 `local_continuous` tick；`destroy_self()`（或过期逻辑）终结它们——生杀命令在 calc 批次结束后统一提交，故同帧内读保持快照一致。关键在于这**不削弱单向依赖铁律与 D1**：local 通道仍按同样三类注册（`local_continuous` 跑逐帧运动 / 衰减，受字段级单写者纪律约束；`local_renderable` 给提交绑定），并经独立的 `submit_local()` / `LocalSubmissionView` 提交——后端可把粒子走自己的 pass，无需伪造 `InstanceId`。**这不是第四种概念**，而是把既有 render 机器指向一个 render 自持的池，而非 sim sidecar。
